...
abbrev_file_name = os.path.join(current_dir, 'us_state_abbrev.json')
us_state_abbrev = json.load(open(abbrev_file_name))


def get_daily_data(state, dates, columns=None):
    """
    :param dates: a string or a list of string to indicate the dates
    :param state: the two-letter state shorthand
...
def get_daily_data_by_county(counties=None, states=None, dates=None, fips=None):
    ...
    :param dates:
    :param fips: a unique county identifier
    ...
    note that both a list or a string would work for most cases
        >>> get_daily_data_by_county(counties='Richland',
                                     states='SC',
                                     dates='2020-12-24')
        >>> get_daily_data_by_county(counties=['Richland', 'Charleston'],
                                     states='SC',
                                     dates=['2020-12-24', '2020-12-25'])
    """
    counties = [counties] if isinstance(counties, str) else counties
    fips = [fips] if isinstance(fips, str) else fips
    ...
    for idx, entry_ in tqdm(enumerate(content)):
        if idx != 0:
            row = entry_.split(',')
            rows.append(row)
        else:
    ...
    if fips:
        covid_df = covid_df[covid_df['fips'].isin(fips)]
...
def get_daily_data_by_zip_code(states=None, zip_codes=None, dates=None):
    ...
    lookup_table = _get_fips_to_zips_dict()
    ...
    for zip_code in zip_codes:
        zip_code_string, _, _ = lookup_table[zip_code]
        fips_list.append(zip_code_string)

    return get_daily_data_by_county(states=states, dates=dates, fips=fips_list)
